...
The workspace covers the two-stage Helion FX pipeline described in `project.plan.md`. Keep Python capture code in `partA-helion2json/helion2json/` and its tests in `partA-helion2json/tests/`. C++ lowering lives in `partB-json2mlir/include/helion_mlir/` for headers, `partB-json2mlir/lib/` for implementations, and `partB-json2mlir/tests/FileCheck/` for golden MLIR fixtures. Place shared JSON samples under `samples/` and keep experimental notebooks or scratch files outside the repo to avoid noise.
...
- `pip install -r requirements.txt` pulls PyTorch, Triton, and Helion with the right CUDA/ROCm wheels; add `-e partA-helion2json` when the package metadata lands.
- `cmake -S partB-json2mlir -B build -G Ninja -DLLVM_DIR=/path/to/llvm` configures MLIR tooling; pass `-DTORCH_MLIR_HOME` when the torch dialect is required.
- `ninja -C build json2mlir` builds the CLI; `ninja -C build check-json2mlir` runs FileCheck tests; use `pytest partA-helion2json/tests` for the Python suite.
