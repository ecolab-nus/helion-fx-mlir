...
├── helion_mlir.py           # Entry point: generate_mlir(), validate_with_mlir_opt()
...
- `torch.aten.*` operations (ATen compute ops via torch-mlir)

```bash
mlir-opt -allow-unregistered-dialect output.mlir
```

The `validate_with_mlir_opt()` function automates this by searching for `mlir-opt` in common locations.
...
- **Unregistered Dialects**: `loom.*` and `torch.*` ops require `-allow-unregistered-dialect` flag
