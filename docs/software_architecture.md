...
2.  **Torch functions (`aten`)** are generated by the **torch-mlir project**.
3.  The generated MLIR is a mix of dialects including **`affine.parallel`, `affine.for`, `tensor`, `linalg`**, and `helion`.
...
└── lowerings/               # Modular lowering implementations
    ...
    ├── aten_ops.py          # ATen op lowerings (delegates to torch-mlir/linalg)
...
- Configured to emit `linalg-on-tensors` dialect for compute operations.
...
|----------------|-----------|----------------------|
| `aten.addmm` | **torch-mlir** | `linalg.matmul` |
| `aten.relu` | **torch-mlir** | `linalg.generic` / `arith` |
